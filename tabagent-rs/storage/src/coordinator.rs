...
mod tests {
    ...
    fn test_insert_and_get_message() {
        let coordinator = DatabaseCoordinator::new().unwrap();
        
        let message = Message {
            id: NodeId::from("msg_test"),
            chat_id: NodeId::from("chat_1"),
            sender: "user".to_string(),
        ...
        coordinator.insert_message(message.clone()).unwrap();
        
        let retrieved = coordinator.get_message("msg_test").unwrap();
        assert!(retrieved.is_some());
        assert_eq!(retrieved.unwrap().id.as_str(), "msg_test");
    }
    ...
    fn test_insert_and_get_entity() {
        ...
        coordinator.insert_entity(entity.clone()).unwrap();
        
        let retrieved = coordinator.get_entity("entity_test").unwrap();
        assert!(retrieved.is_some());
        assert_eq!(retrieved.unwrap().label, "Test Entity");
    }
}
